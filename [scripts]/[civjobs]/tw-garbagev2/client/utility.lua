        ...
        while not shouldExit and isAnimRunning do
            if not IsEntityPlayingAnim(ped, 'anim@heists@narcotics@trash', 'walk', 3) then
                if not IsEntityPlayingAnim(ped, 'anim@scripted@freemode@postertag@collect_can@male@', 'poster_tag_collect_can_var03_male', 3) and
                    not IsEntityPlayingAnim(ped, 'anim@mp_fireworks', 'place_firework_box2', 3) then
        ...
        if not deliveryThread then
            ...
            Citizen.CreateThread(function()
                while deliveryThread do
                    ...
                    if isPlayerInRegisteredVehicle and isOwner and IsPedInAnyVehicle(playerPed, false) and not isInteracting then
                        ...
                        if dist < 10.0 then
                            ...
                            if IsControlJustReleased(0, tonumber(Config.InteractionKeys['E'])) then -- E tuşu
                                StartInteraction()
...
function ShowUniversalInteraction(interactionId, coords, text, distance, interactionOptions)
    ...
    if interactionOptions and interactionOptions[1] and interactionOptions[1].onSelect and
        ...
        IsControlJustPressed(0, tonumber(interactionId)) and not IsPedInAnyVehicle(PlayerPedId(), false) then
