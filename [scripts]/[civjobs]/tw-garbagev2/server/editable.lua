        ...
        embeds = {
            {
                ...
                color = 0xFFA500,
                author = {
                    name = 'Tworst  Garbage - JOB FINISH',
                },
                thumbnail = {
                    url = data.avatar
                },
    ...
    if lobby.roomSetting then
        for _, player in ipairs(lobby.players) do
            if player.source == src then
                ...
                local function deleteEntityByNetId(tableData)
                    if tableData then
                        for i = 1, #tableData do
                            local box = tableData[i]
                            local boxObject = NetworkGetEntityFromNetworkId(box.objectNetId)
                            if DoesEntityExist(boxObject) then
                ...
                for _, tableName in ipairs(tablesToClear) do
                    if lobby.roomSetting.Mission and lobby.roomSetting.Mission[tableName] then
                        deleteEntityByNetId(lobby.roomSetting.Mission[tableName])
                    end
                end
                ...
                if occupiedCoords[owneridentifier] then
                    occupiedCoords[owneridentifier] = nil
                end
