...
function selectRandomCoords(coordsList, count, owneridentifier)
    local selected = {}
    local availableCoords = {} -- Her çağrıda yeni tablo oluştur

    -- Tüm oyuncuların occupiedCoords'larını birleştir
    local allOccupiedCoords = {}
    for playerIdentifier, playerCoords in pairs(occupiedCoords or {}) do
        for _, coord in ipairs(playerCoords) do
            table.insert(allOccupiedCoords, coord)
        end
    end

    for _, coord in ipairs(coordsList) do
        local isOccupied = false
        for _, occupied in ipairs(allOccupiedCoords) do
            local occupiedVector = vector3(occupied.x, occupied.y, occupied.z)
            local coordVector = vector3(coord.x, coord.y, coord.z)
            local distance = #(occupiedVector - coordVector)
        ...
        if not isOccupied then
            table.insert(availableCoords, coord)
        end
    end

    if #availableCoords < count then
        return nil, false
    end

    math.randomseed(os.time())
    for i = 1, count do
        local index = math.random(1, #availableCoords)
        table.insert(selected, table.remove(availableCoords, index))
    end
