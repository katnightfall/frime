...
Config.CompatibleInventoryScripts = { -- Having a compatible inventory script is required
    "ox_inventory",
    "qb-inventory",
    "qs-inventory",
    "ps-inventory",
...
Config.CompatibleDoorLockScripts = { -- Having a compatible door lock script is required
    "qb-doorlock",
    "ox_doorlock",
    "rcore_doorlock",
}
...
-- (DONT TOUCH ANY OF THIS SECTION)
for _, v in pairs(Config.CompatibleDoorLockScripts) do
    if GetResourceState(v) == 'started' then
        Config.DoorLock = tostring(v)
        break
    end
end
------------------------------
