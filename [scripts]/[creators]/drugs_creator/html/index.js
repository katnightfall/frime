...
async function loadHarvestableItems() {
    const rawHarvestableItems = await $.post(`https://${resName}/getAllHarvestableItems`);

    harvestableItems = {};

    Object.entries(rawHarvestableItems).forEach(([_, itemData]) => {
        harvestableItems[itemData.id] = itemData;
    });

    const rows = await Promise.all(
        Object.entries(harvestableItems).map(async ([itemId, itemData]) => [
            itemId,
    ...
    // Add all rows at once and draw once
    harvestableItemsDatatable.clear().rows.add(rows).draw();
}
...
async function getItemLabel(itemName) {
    const itemLabel = await $.post(`https://${resName}/getItemLabel`, JSON.stringify({itemName}));
...
async function addItemEffect(itemName, itemData={}, isNew = false) {
			...
			<div class="effects mt-3">
				...
				<div class="d-flex row-cols-3 flex-wrap">
					<div class="d-flex align-items-center gap-3 my-1 cumulative-effect" data-cumulative-name="armor">
						...
						<div class="form-floating col-3">
							<input type="number" min=1 class="form-control cumulative-effect-amount" placeholder="...">
	...
	// Set all disabled for cumulative effects radio and amount input
	itemEffectDiv.find(".cumulative-effect").find("input[type=radio]").prop("disabled", true);
	itemEffectDiv.find(".cumulative-effect").find(".cumulative-effect-amount").prop("disabled", true);
	...
	// Add event listeners for cumulative effects
	itemEffectDiv.find(".cumulative-effect-checkbox").on("change", function() {
		const cumulativeEffect = $(this).closest(".cumulative-effect");
		const isChecked = $(this).prop("checked");
		
		cumulativeEffect.find("input[type=radio]").prop("disabled", !isChecked);
		
		if (!isChecked) {
			cumulativeEffect.find("input[type=radio]").prop("checked", false);
			cumulativeEffect.find(".cumulative-effect-amount").prop("disabled", true);
			cumulativeEffect.find(".cumulative-effect-amount").val("");
		}
	});

	itemEffectDiv.find("input[type=radio]").on("change", function() {
		const cumulativeEffect = $(this).closest(".cumulative-effect");
		const hasCheckedRadio = cumulativeEffect.find("input[type=radio]:checked").length > 0;
		
		cumulativeEffect.find(".cumulative-effect-amount").prop("disabled", !hasCheckedRadio);
	});

	if(itemData.cumulativeEffects) {
		for(let cumulativeEffect of itemData.cumulativeEffects) {
			const effectDiv = itemEffectDiv.find(`.cumulative-effect[data-cumulative-name=${cumulativeEffect.type}]`);
			effectDiv.find(".cumulative-effect-checkbox").prop("checked", true).change();
			effectDiv.find(`input[name=drug-cumulative-${cumulativeEffect.type}-${itemName}][value=${cumulativeEffect.action}]`).prop("checked", true).change();
			effectDiv.find(".cumulative-effect-amount").val(cumulativeEffect.amount);
		}
	}
