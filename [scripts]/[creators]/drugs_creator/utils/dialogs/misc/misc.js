...
// Used in disabled-input class, readonly but still validated
document.addEventListener('click', (e) => {
	if (e.target.matches('input.disabled-input')) {
		e.preventDefault();
		e.target.blur();
	}
});
...
function teleportToCoords(coords = {}, heading) {
	if(!coords.x || !coords.y || !coords.z) return;

	$.post(`https://${resName}/teleportToCoords`, JSON.stringify({coords, heading}));
}
...
function getFirstActiveDialog() {
	const activeDialogs = $(".modal.show");
	if(activeDialogs.length == 0) return null;

	let highestZIndex = 0;
	let firstActiveDialog = null;
	activeDialogs.each(function(i, element) {
		element = $(element);

		const zIndex = parseInt(element.css("z-index"));
		if(zIndex > highestZIndex) {
			highestZIndex = zIndex;
			firstActiveDialog = element;
		}
	});
...
document.addEventListener('keyup', (event) => {
	...
	const firstActiveDialog = getFirstActiveDialog();
	if(firstActiveDialog) {
		firstActiveDialog.modal("hide");
	} else {
		closeMenu();
	}
});
