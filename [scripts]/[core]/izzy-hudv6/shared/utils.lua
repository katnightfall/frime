...
if cfg?.framework then
    ...
    local qbcore = GetResourceState('qb-core'):find('start') or GetResourceState('qbx-core'):find('start') or GetResourceState('qbx_core'):find('start')
    if esx then
        framework = 'esx'
    elseif qbcore then
        framework = 'qb'
...
if not IsDuplicityVersion() then --client
    ...
    izzy.callback = function (name, ...)
        ...
        local handler = RegisterNetEvent(('izzy-events-%s:%s'):format(GetCurrentResourceName(), name), function (...)
            p:resolve({...})
        end)
    ...
    izzy.createCallback = function (name, fn)
        RegisterNetEvent(('izzy-events-%s:%s'):format(GetCurrentResourceName(), name), function (id, ...)
            local src = source
            TriggerClientEvent(('izzy-events-%s:%s'):format(GetCurrentResourceName(), name), src, fn(src, ...))
        end)
    end
