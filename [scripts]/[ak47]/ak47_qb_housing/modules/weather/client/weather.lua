Citizen.CreateThread(function()
    if Config.Weather.script == 'auto' then
        ...
        if GetResourceState('av_weather') == 'started' or GetResourceState('av_weather') == 'starting' then
            Config.Weather.script = 'av_weather'
            return
        end
...
AddEventHandler('ak47_qb_housing:pausesync', function(pause, time)
    ...
    elseif Config.Weather.script == 'av_weather' then
        TriggerEvent('av_weather:freeze', pause, 0, 0, Config.Weather.defaultweather)
    end
end)
...
function SetInteriorTime(time)
    if not DoorCam and Config.Weather.script ~= 'qb-weathersync' and Config.Weather.script ~= 'av_weather' then
