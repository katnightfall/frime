...
function getSocietyMoney()
    if GetResourceState('okokBanking') == 'started' then
        return exports['okokBanking']:GetAccount('cannabiscafe')
    end
    if Config.SocietyInManagement then
        return exports['qb-management']:GetAccount('cannabiscafe')
    else
        return exports['qb-banking']:GetAccountBalance('cannabiscafe')
    end
end

function addSocietyMoney(money)
    if GetResourceState('okokBanking') == 'started' then
        exports['okokBanking']:AddMoney('cannabiscafe', money)
        return
    end
    if Config.SocietyInManagement then
        exports['qb-management']:AddMoney('cannabiscafe', money)
    else
        exports['qb-banking']:AddMoney('cannabiscafe', money)
    end
end

function removeSocietyMoney(money)
    if GetResourceState('okokBanking') == 'started' then
        exports['okokBanking']:RemoveMoney('cannabiscafe', money)
        return
    end
    if Config.SocietyInManagement then
        exports['qb-management']:RemoveMoney('cannabiscafe', money)
    else
        exports['qb-banking']:RemoveMoney('cannabiscafe', money)
    end
end
...
RegisterServerEvent('ak47_qb_cannabiscafev2:PlayWithinDistance')
AddEventHandler('ak47_qb_cannabiscafev2:PlayWithinDistance', function(coords , maxDistance, soundFile)
    TriggerClientEvent('ak47_qb_cannabiscafev2:PlayWithinDistance', -1, coords, maxDistance, soundFile)
end)